...
def final_models(model,Xr_train,Xw_train,yr_train,yw_train,Xr_val,Xw_val,yr_val,yw_val):
    ...
    if model == 'lr':
        # features
        fr=['volatile_acidity_s', 'residual_sugar_s', 'chlorides_s', 'free_so2_s', 'total_so2_s', 'pH_s', 'sulphates_s', 'alcohol_s']
        ...
        # features
        fr=['volatile_acidity_s', 'citric_acid_s', 'residual_sugar_s', 'chlorides_s', 'free_so2_s', 'total_so2_s', 'density_s', 'pH_s', 'sulphates_s', 'alcohol_s']
...
def plt_err(Xr_train,Xw_train,yr_train,yw_train,Xr_test,Xw_test,yr_test,yw_test):
    ...
    plt.annotate("Baseline: Mean", (3.2, 5.9))
    plt.plot(yr_test, yr_test, alpha=.5, color="red", label='_nolegend_')
    plt.annotate("Ideal: Predicted = Actual", (3, 2.5), rotation=24)
